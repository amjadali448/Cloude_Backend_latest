...
app.post('/initiate-docker-build', async (req, res) => {
  ...
  try {
    ...
    const buildImageApi = `http://${ipAddress}:2375/build?t=${tag}`;
...
app.post('/tag-image', async (req, res) => {
  ...
  try {
    const imageName = tagName;
      ...
      //   return res.status(400).json({ error: 'Image Name is required' });
      // }
      const targetTag = req.query.tagName;
...
app.post('/Disconnect-container', async (req, res) => {
  ...
  try {
    const disconnectConatinerApi = `http://${ipAddress}:2375/networks/${Container}/disconnect`;
